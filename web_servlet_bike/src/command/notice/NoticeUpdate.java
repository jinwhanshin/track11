...
public class NoticeUpdate implements CommonExcute {
	...
	public void execute(HttpServletRequest request) {
		...
		String deleteAttach 
					= mpr.getParameter("t_deleteCheckbox");
		boolean delAttach = true;
		if(deleteAttach != null){
			File file = 
					new File(CommonUtil.getFile_dir_notice(),deleteAttach);
		...
		if(!attach.equals("")){
			if(!oriAttach.equals("") && delAttach){
				File file = 
					new File(CommonUtil.getFile_dir_notice(),oriAttach);
				boolean tf = file.delete();
				if(!tf) System.out.println("공지사항 기존 첨부파일 삭제~");
			}
		...
		NoticeDto dto = 
				new NoticeDto(no, title, content, saveAttachName);
